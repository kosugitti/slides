...
# はじめに
---
background-image: url(figures/jpaRmd_JPA2021.001.jpeg)
...
```{r bibtex1, echo=FALSE, fig.height=2.7, message=FALSE, warning=FALSE, dev='svglite', out.width='100%'}
...
```{r bibtex2, echo=FALSE, fig.height=2.7, message=FALSE, warning=FALSE, dev='svglite', out.width='100%'}
...
```{r bibtex3, echo=FALSE, fig.height=2.7, message=FALSE, warning=FALSE, dev='svglite', out.width='100%'}
...
+ bib ファイルの文字コードはUTF-8にしてください。Shift-JISだと文字化けしてうまく動きません。
...
+ BibTeXの仕組みは，まず@マークで始まり，中括弧で括られているところまでで1セットになっています。
+ 中身（エントリ）はカンマで区切られていることを確認してください。
  + ネットから取ってきたBib形式のファイルは，そのままでは使えないものがほとんどで，加筆修正の必要があります。その際，「カッコで括る」「カンマで区切る」というファイル形式を損ねると，jpaRmdが動かなくなりますので，この基本をしっかり踏まえておいて欲しいのです。
  <!-- + Mendeley/Zoteroなどの文献管理ソフトが出力するBibファイルもjpaRmd用に加工する必要がある場合がほとんどです。 -->

```{r bibtexp1, echo=FALSE, fig.height=2.7, message=FALSE, warning=FALSE, dev='svglite', out.width='100%'}
...
+ jpaRmdではBook(書籍)，Article(論文)，inCollection(本の一部)，InProceedings(学会発表抄録集)に対応しています。それ以外のカテゴリーのものがあれば，対応しているカテゴリーに書き直してください。
...
```{r bibtexp2, echo=FALSE, fig.height=2.7, message=FALSE, warning=FALSE, dev='svglite', out.width='100%'}
...
+ この例では，本文(Rmd)中に**＠浅見 祐香202192.19053**と書くことで引用できることになります。
  + 自動的に作られるこの引用キーはわかりにくく長いことが多いので，自分の好きな名称に書き換えましょう。
...
+ このあと，これらのファイルをjpaRmdで使えるように加工していきます！ネットで取ってきたもの，Mendeley,Zoteroなどアプリが出力したものが，必ずしも完璧で適切な形ではありませんので，ほぼ確実にBibファイルを加工する必要があります。

---

# BibTeXを加工する1; Author

+ まずは著者情報, Authorのところです。
...
+ 団体名など，「姓名」の形になっていない場合は，{}で括ることで対応します。
...
```{r , echo=FALSE, fig.height=2.7, message=FALSE, warning=FALSE, dev='svglite', out.width='100%'}
knitr::include_graphics("figures/ref.png")
```
...
+ 手弁当でやっていることですが，エラーや要望があれば速やかに対応しますので，是非ご一報を。
