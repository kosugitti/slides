...
+ 中身（エントリ）はカンマで区切られていることを確認してください。
  + ネットから取ってきたBib形式のファイルは，そのままでは使えないものがほとんどで，加筆修正の必要があります。その際，「カッコで括る」「カンマで区切る」というファイル形式を損ねると，jpaRmdが動かなくなりますので，この基本をしっかり踏まえておいください。
...
+ 論文が書き上がった！と思ってもエラーが出てPDFが出力されない，ということがあります。
+ エラーが出た時にチェックすべきことは，
  + 引用がなくても出るエラーかどうかの確認
    + 引用部分をコメントアウトし，本文のエラー出ないかどうか確認してください
    + 引用部分が入るとエラーが起きるようであればBibファイル側の問題です
  + 1レコードずつ追加して行って出力されるかどうか見てみましょう
  + カッコの対応，カンマが抜けてないかなど基本書式の確認を忘れずに
